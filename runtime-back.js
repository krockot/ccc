var Ccc = (function () {
    ...
    Ccc.Object = (function () {
        ...
        Object.prototype.transform = function (syntaxEnvironment, environment) {
            return this;
        };
    ...
    Ccc.Symbol = (function () {
        ...
        Symbol.prototype.getFreeReferences = function (environment) {
            return environment.tryGetLocation(this.id) ? [this.id] : [];
        };
        
        Symbol.prototype.transform = function (syntaxEnvironment, environment) {
            return this;
        };
    ...
    Ccc.True = new Ccc.Boolean(true);
    Ccc.False = new Ccc.Boolean(false);
    ...
    Ccc.Pair = (function () {
        ...
        Pair.prototype.transform = function (syntaxEnvironment, environment) {
            var spec, expansion, o, result = [], innerEnv;
            if (this.car instanceof Ccc.Symbol) {
                spec = syntaxEnvironment.tryGet(this.car.id);
                if (spec instanceof Ccc.TransformerSpec) {
                    expansion = spec.expand(this, syntaxEnvironment, environment);
                    return expansion.transform(syntaxEnvironment, environment);
                }
            }
            o = this;
            while (o instanceof Ccc.Pair) {
                result.push(o.car.transform(syntaxEnvironment, environment));
                o = o.cdr;
            }
            if (o !== Ccc.Nil) {
                o = o.transform(syntaxEnvironment, environment);
            }
    ...
    Ccc.Program = (function () {
        ...
        Program.prototype.transform = function (syntaxEnvironment, environment) {
            var i, data = [];
            for (i = 0; i < this.data.length; i += 1) {
                data[i] = this.data[i].transform(syntaxEnvironment, environment);
            }
    ...
    // and optionally may have a parent environment which is recursively used
    // to resolve unknown bindings.
    Ccc.Environment = (function () {
        var builtins = {};
        
        var Environment = function (outer) {
            var k, bindings = {}, gensymId = 0, self = this;
            ...
            this.gensym = function () {
                while (self.tryGetLocation("\x7f#" + gensymId)) {
                    gensymId += 1;
                }
                self.reserve("\x7f#" + gensymId);
                return "\x7f#" + gensymId;
            };
        ...
        Environment.defineBuiltin = function (id, arity, fn, returnContinuation) {
            var i, ids = [];
            ...
            builtins[id] = { value: new Ccc.BuiltinProcedure(id, fn, arity, returnContinuation) };
            for (i = 0; i < ids.length; i += 1) {
                builtins[ids[i]] = { value: builtins[id].value };
            }
        };
        
        Environment.defineLibrary = function (library) {
            ...
            for (name in library) {
                if (library.hasOwnProperty(name)) {
                    ...
                    if (!(o instanceof String)) {
                        ...
                        Environment.defineBuiltin(name, arity, fn);
                    }
                }
            }
            for (name in aliases) {
                if (aliases.hasOwnProperty(name)) {
                    if (!builtins.hasOwnProperty(aliases[name])) {
                        throw new Error("Unable to alias non-existent builtin '" + aliases[name] + "'");
                    }
                    builtins[name] = { value: builtins[aliases[name]].value };
                }
            }
        };
    ...
    // The standard R5RS environment
    Ccc.ReportEnvironment = new Ccc.Envrionment();
    ...
    Ccc.TransformerSpec = (function () {
        ...
        TransformerSpec.prototype.expand = function (datum, syntaxEnvironment, environment) {
            var i, k, result;
            for (i = 0; i < this.rules.length; i += 1) {
                result = this.rules[i].tryExpand(this.literals, datum, syntaxEnvironment, environment);
                if (result !== null) {
                    return result;
                }
            }
    ...
    Ccc.SyntaxRule = (function () {
        ...
        SyntaxRule.prototype.tryExpand = function (literals, datum, syntaxEnvironment, environment) {
            var captures, expansion;
            captures = this.pattern.match(literals, datum.cdr, environment);
            if (captures !== false) {
                if (this.template instanceof Function) {
                    expansion = this.template(captures, syntaxEnvironment, environment);
                }
                else {
                    expansion = this.template.expand(captures, syntaxEnvironment, environment);
                }
                if (expansion === null) {
    ...
    Ccc.Transformer = (function () {
        var Transformer = function () {
            this.syntaxEnvironment = new Ccc.Environment();
            this.environment = new Ccc.Environment();
            
            var syntaxEnv = this.syntaxEnvironment;
            var env = this.environment;
            var D = function (n, ts) { syntaxEnv.define(n, ts); };
            ...
            var P = function (d) { return new Ccc.Pattern(d); };
            ...
            D("define-syntax", TS([], [
                R(L(S("keyword"), S("transformer-spec")),
                    function (captures, syntaxEnvironment, environment) {
                        var sym = captures["keyword"];
                        syntaxEnv.define(sym.id, captures["transformer-spec"].transform(syntaxEnvironment, environment));
            ...
            D("let-syntax", TS([], [
                R(L(L(E(L(S("keyword"), S("transformer-spec")))), E(S("body"))),
                    function (captures, syntaxEnvironment, environment) {
                        var newEnv = syntaxEnvironment.clone(),
                            syms = _map(captures["keyword"].datum.toArray(), function (id) { return id; }),
                            transformers = _map(captures["transformer-spec"].datum.toArray(), function (t) { return t.transform(syntaxEnvironment, environment); }),
                            i, j, body, replacements = {}, freeRefs, outerSymbols = [], argsList, argsVals;
                        for (i = 0; i < syms.length; i += 1) {
                            freeRefs = transformers[i].getFreeReferences(environment);
                            for (j = 0; j < freeRefs.length; j += 1) {
                                if (!replacements.hasOwnProperty(freeRefs[j])) {
                                    replacements[freeRefs[j]] = new Ccc.Symbol(environment.gensym(), freeRefs[j]);
                                }
                            }
                            transformers[i].replaceSymbols(replacements);
                            newEnv.define(syms[i].id, transformers[i]);
                        }
                        ...
                        body = _map(captures["body"].datum.toArray(), function (b) { return b.transform(newEnv, environment); });
                        return new Ccc.Pair(new Ccc.Pair(new Ccc.Symbol("lambda"), new Ccc.Pair(argsList, Ccc.Pair.fromArray(body))), argsVals);
                    })]));
                ...
                R(L(L(E(L(S("keyword"), S("transformer-spec")))), E(S("body"))),
                    function (captures, syntaxEnvironment, environment) {
                        var newEnv = syntaxEnvironment.clone(),
                            syms = _map(captures["keyword"].datum.toArray(), function (id) { return id; }),
                            transformers = _map(captures["transformer-spec"].datum.toArray(), function (t) { return t.transform(syntaxEnvironment, environment); }),
                            i, j, body, keywords = {}, replacements = {}, freeRefs, outerSymbols = [], argsList, argsVals;
                        for (i = 0; i < syms.length; i += 1) {
                            keywords[syms[i].id] = {};
                        }
                        for (i = 0; i < syms.length; i += 1) {
                            freeRefs = transformers[i].getFreeReferences(environment);
                            for (j = 0; j < freeRefs.length; j += 1) {
                                if (!replacements.hasOwnProperty(freeRefs[j]) && !keywords.hasOwnProperty(freeRefs[j])) {
                                    replacements[freeRefs[j]] = new Ccc.Symbol(environment.gensym(), freeRefs[j]);
                                }
                            }
                            transformers[i].replaceSymbols(replacements);
            ...
            D("define", TS([], [
                R(L(L(S("v"), E(S("args"))), E(S("body"))),
                    function (captures, syntaxEnvironment, environment) {
                        var args = _map(captures["args"].datum.toArray(), function (a) { return a; });
                        var body = _map(captures["body"].datum.toArray(), function (b) { return b.transform(syntaxEnvironment, environment); });
                ...
                R(L(S("v"), S("e")),
                    function (captures, syntaxEnvironment, environment) {
                        return new Ccc.Define(captures["v"], captures["e"].transform(syntaxEnvironment, environment));
                    }),
                    ]));
            ...
            D("lambda", TS([], [
                R(L(S("formals"), E(S("body"))),
                    function (captures, syntaxEnvironment, environment) {
                        ...
                        var body = _map(captures["body"].datum.toArray(), function (b) { return b.transform(syntaxEnvironment, environment); });
                        return new Ccc.Lambda(args, rest, Ccc.Pair.fromArray(body));
                    })]));
                    
            D("if", TS([], [
                R(L(S("test"), S("consequent"), S("alternate")),
                    function (captures, syntaxEnvironment, environment) {
                        var test = captures["test"].transform(syntaxEnvironment, environment);
                        var consequent = captures["consequent"].transform(syntaxEnvironment, environment);
                        var alternate = captures["alternate"].transform(syntaxEnvironment, environment);
            ...
            D("set!", TS([], [
                R(L(S("symbol"), S("expr")),
                    function (captures, syntaxEnvironment, environment) {
                        return new Ccc.Assignment(captures["symbol"], captures["expr"].transform(syntaxEnvironment, environment));
                    })]));
                    
            // Load builtin keywords
            var self = this;
            $.get("js/ccc/keywords.scm").success(function (data) {
                var program = Ccc.Parser.parse(data);
                self.transform(program);
            }).error(function () {
                throw new Error("Unable to load builtin syntactic keywords");
            });
        };
        
        Transformer.prototype.transform = function(program) {
            return program.transform(this.syntaxEnvironment, this.environment);
        };
    ...
    Ccc.Compiler = (function () {
        var Compiler = function (options) {
            ...
            this.transformer = new Ccc.Transformer();
        };
        
        Compiler.prototype.compile = function (code) {
            var program, start, compiled, transformed, transformer;
    ...
    Ccc.Vm = (function () {
        var Vm = function (options) {
            var ops, env = new Ccc.Environment(), step, executeBlock;
