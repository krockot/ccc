(function() {
  ...
  ccc.lib.base.addSimpleFunctions({
    ...
    "make-string": function(count, char) {
      requireNumber(count, 0, "make-string");
      requireChar(char, 1, "make-string");
      return new ccc.String((Array(count.value_ + 1)).join(String.fromCharCode(char.charCode_)));
    },
    ...
    "list->string": function(list) {
      ...
      list.forEach(
        function(char) {
          if (char.constructor !== ccc.Char)
            throw new Error("list->string: Expected list of characters");
          string += String.fromCharCode(char.charCode_);
        },
    ...
    "string-fill!": function(string, char) {
      requireString(string, 0, "string-fill!");
      requireChar(char, 1, "string-fill!");
      string.value_ = (Array(string.value_.length + 1)).join(String.fromCharCode(char.charCode_));
    },
  });
  ...
  var addCIStringComparator = function(name, comparator) {
    ccc.lib.base.addSimpleFunction(name, function(string1, string2) {
      requireString(string1, 0, name);
      requireString(string2, 1, name);
      return comparator(string1.value_.toLocaleLowerCase(), string2.value_.toLocaleLowerCase())
             ? ccc.t : ccc.f;
    });
  };
